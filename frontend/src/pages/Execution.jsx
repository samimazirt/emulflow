...
import { getAppliances, createTest } from '../api';
...
const TestExecution = () => {
    ...
    const createTestMutation = useMutation({
        ...
        onSuccess: (data) => {
            ...
            navigate(`/tests/${data.data.id}`); // Redirect to the new test's detail page
        },
        onError: (error) => {
            ...
            <div className="card bg-base-200 shadow-xl">
                <div className="card-body space-y-6">
                    ...
                    <div className="form-control w-full">
                        <label className="label"><span className="label-text font-bold text-lg">1. Select Appliances</span></label>
                            ...
                            {appliancesLoading ? (
                                ...
                                !appliancesData?.data || appliancesData.data.length === 0 ? (
                                    <div className="text-center p-4 flex flex-col items-center justify-center h-full">
                                        ...
                                        <Link to="/configuration" className="btn btn-xs btn-primary">Add an Appliance</Link>
                                    </div>
                                ) : (
                                    appliancesData.data.map(appliance => (
                                        <label key={appliance.id} className="label cursor-pointer hover:bg-base-300 rounded">
                                            <span className="label-text">{appliance.name} ({appliance.ip_address})</span>
                                            <input
                                                type="checkbox"
                    ...
                    <div className="form-control w-full">
                        <label className="label"><span className="label-text font-bold text-lg">2. Set Intensity</span></label>
                    ...
                    <div className="card-actions pt-4">
                        ...
                        >
                            {createTestMutation.isLoading ? <span className="loading loading-spinner"></span> : 'Launch Test'}
                        </button>
                    </div>
                </div>
            </div>
